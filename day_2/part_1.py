...
def get_game_sum(game_id: int, sets_cubes: []):
    for cubes in sets_cubes:
        if cubes[Cubes.RED.value] > MATCH_GAME[Cubes.RED.value]:
            return 0
        if cubes[Cubes.BLUE.value] > MATCH_GAME[Cubes.BLUE.value]:
            return 0
        if cubes[Cubes.GREEN.value] > MATCH_GAME[Cubes.GREEN.value]:
            return 0
    return game_id
